...
public class ActivityDBAO {
	...
	public List<Activity> getRecentActivity() throws SQLException {
		...
		try {
			String sqlStatement = "select a.id, a.name, a.desc, a.startTime, a.endTime, a.category, a.count, a.image from activities as a where id in (select t.id from (select id from activities where  category = 'music'  order by createdAt desc  limit 3) as t)or id in (select t.id from (select id from activities where  category = 'lecture' order by createdAt desc  limit 3) as t)or id in (select t.id from (select id from activities where  category = 'party' order by createdAt desc  limit 3) as t) or id in (select t.id from (select id from activities where  category = 'movie' order by createdAt desc  limit 3) as t) or id in (select t.id from (select id from activities where  category = 'exhibition' order by createdAt desc  limit 3) as t) or id in (select t.id from (select id from activities where  category = 'sport' order by createdAt desc  limit 3) as t)or id in (select t.id from (select id from activities where  category = 'travel' order by createdAt desc  limit 3) as t)or id in (select t.id from (select id from activities where  category = 'others' order by createdAt desc limit 3) as t)";
			PreparedStatement prepStmt = con.prepareStatement(sqlStatement);
			ResultSet rs = prepStmt.executeQuery();
	...
	public Activity getActivityById(String id) throws Exception {
		...
		try {
			...
			List<User> likeusers = db_1.getLikeusersByActivityId(id);
			...
			if(rs.next()) {
				activity = new Activity(rs.getString(1), 
						...
						rs.getString(8), rs.getInt(9), rs.getInt(10), rs.getString(11), rs.getString(12), comments, rs.getString(13), users, likeusers);
			}
			prepStmt.close();
		} catch(SQLException e) {
			e.printStackTrace();
